export default function (services) {
    ...
    mainModel.properties.Project.config.active = createExpression([step1Model.properties.Type], (t) => t === 'project');
    ...
    return {
        ...
        async initialize() {
            let response = await rest.executeGetQuery({ url: '/api/rpt00701-nieuwe-verkoopprijs' });
            let dateProps = new Set(['Begindatum', 'Einddatum']);
            for (let [key, value] of Object.entries(response)) {
                let prop = mainModel.properties[key] ?? step1Model.properties[key];
                if (prop) {
