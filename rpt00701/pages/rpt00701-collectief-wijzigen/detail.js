export default function (services) {
    ...
    mainModel.properties.VastBedrag.config.locked = createExpression([mainModel.properties.Percentage], (p) => !!p);
    mainModel.properties.OverNemenVanArtikel.config.locked = createExpression([mainModel.properties.Percentage, mainModel.properties.VastBedrag], (p, v) => !!p || !!v);
    // Vast bedrag gevuld → Percentage locked
    mainModel.properties.Percentage.config.locked = createExpression([mainModel.properties.VastBedrag], (v) => !!v);
    ...
    return {
        ...
        async initialize() {
            let response = await rest.executeGetQuery({ url: '/api/rpt00701-collectief-wijzigen' });
