...
export default function (services: BlueprintFactories): WizardPage {
  ...
  return {
    ...
    completeAction: {
      label: 'Voltooien',
      async execute() {
        if (!confirm('Weet je zeker dat je de geselecteerde toekenningen wilt terugdraaien?')) return false;
        return true;
      }
    },
    async initialize(context) {
      let response = await rest.executeGetQuery<Record<string, unknown>>({ url: '/api/rpt00692-terugdraaien-wizard' });
